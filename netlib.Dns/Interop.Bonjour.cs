...
// Source: https://github.com/DisplayCast/Win7/blob/master/ZeroconfService/mDNSImports.cs
namespace netlib.Dns
{
	...
	enum DNSServiceFlags : uint
	{
	     ...
	     * (In earlier builds this flag was briefly called kDNSServiceFlagsReturnCNAME)
	     */
	...
	internal sealed class DNSImports
	{
		static DNSImports()
		{
			if (OperatingSystem.IsMacOS() || OperatingSystem.IsMacCatalyst() || OperatingSystem.IsIOS() || OperatingSystem.IsTvOS())
			{
				NativeLibrary.SetDllImportResolver(
					typeof(DNSImports).Assembly,
					(libraryName, assembly, searchPath) =>
					{
						if (libraryName == "dnssd")
						{
							return NativeLibrary.Load("/usr/lib/system/libsystem_dnssd.dylib", assembly, default);
						}
						return default;
					});
			}
		...
		/// The DNS Service Discovery implementation uses this socket to communicate between
		...
		public delegate void DNSServiceBrowseReply(
			...
			[MarshalAs(UnmanagedType.LPUTF8Str)] string replyDomain,
			IntPtr context);
		...
		public static extern DNSServiceErrorType DNSServiceBrowse(
			...
			DNSImports.DNSServiceBrowseReply callBack,
			IntPtr context);
		...
		public delegate void DNSServiceResolveReply(IntPtr sdRef,
			...
			[MarshalAs(UnmanagedType.LPUTF8Str)] string hosttarget,
			UInt16 port,
			UInt16 txtLen,
		...
		public static extern DNSServiceErrorType DNSServiceQueryRecord(
			...
			DNSServiceType rrType,
			DNSServiceClass rrClass,
			DNSImports.DNSServiceQueryReply callBack,
			IntPtr context);
		...
		public static extern DNSServiceErrorType DNSServiceRegister(
			...
			ushort txtLen,
			byte[] txtRecord,
			DNSImports.DNSServiceRegisterReply callBack,
			IntPtr context);
