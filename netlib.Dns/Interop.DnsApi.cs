    ...
    internal class DnsRequest
    {
        ...
        public DnsResponse GetResponse(DnsRecordType dnstype)
        {
            ...
            try
            {
                ...
                if (ret != 0)
                    throw new DnsException($"DNS query fails with error code {ret}", ret);
		...
		public bool TryGetResponse(DnsRecordType dnstype, [NotNullWhen(true)] out DnsResponse? response)
		{
			response = null;
			if (Environment.OSVersion.Platform != PlatformID.Win32NT)
				return false;
			...
			try
			{
				...
				do
				{
					...
					// Add wrapper to array
					//if (! resp.RawRecords.Contains(wrapper))
					response.RawRecords.Add(wrapper);
